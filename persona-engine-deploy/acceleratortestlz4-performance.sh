...
aws s3 rm s3://personas-dev/customer/acceleratortestlz4/bind/cbind/ --recursive --exclude '*' --include '*.lz4' --include '*SUCCESS' --include '*.bz2'
aws s3 rm s3://personas-dev/customer/acceleratortestlz4/bind/vbind/ --recursive --exclude '*' --include '*.lz4' --include '*SUCCESS' --include '*.bz2'
aws s3 rm s3://personas-dev/customer/acceleratortestlz4/userlocationlog/ --recursive --exclude '*' --include '*.lz4' --include '*SUCCESS' --include '*.bz2'
...
if [ -e acceleratortestlz4-performance`date +'%Y%m%d'` ]; then
    ...
    nohup python ./workflow/persona-runner.py --cfg-file ./workflow/acceleratortestlz4-runner.automation.cfg --start-date $STARTDATE --end-date $ENDDATE --log-level=DEBUG > acceleratortestlz4-performance.out.`date +'%Y%m%d'` 2>&1
...
do
	...
	nohup python ./home_personify.py --cfg-file=acceleratortestlz4_personify.properties --bind-date=$i > acceleratortestlz4.$i.log 2>&1
